...
class trmiAnalytics:
    ...
    def getLinearRegression(self, dfp, dataSpan):
        ...
        Y = dfp[len(dfp) - dataSpan - 1:len(dfp) - 1]['lastPrice']
        print (Y)
        clf.fit(X,Y)
        print (clf.coef_)
    ...
    def getSupervisingData(self, dfp, dft1, dft2, dft3, numDepth, flag):
    ...
    def getSupervisingMLPData(self, dfp, dft1, dft2, dft3, activationFunc, flag):
        # SUPPERVISOR DATA CREATION
        activationFunction = activationFunc.split('-')[0]
        solverName = activationFunc.split('-')[1]
        X = []
        sSize = len(dfp)
        P = []
        P = dfp['lastPrice']
        ...
        clf = MLPClassifier(activation=activationFunction, solver=solverName, random_state=0,max_iter=10000)
        clf = clf.fit(T,X)
