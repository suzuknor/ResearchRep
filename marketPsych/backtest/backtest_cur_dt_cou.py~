...
argc=len(argvs)
if argc != 6:
    print ('Usage: # python %s Target_Asset Number_of_Depth_for_DecisionTree start_from span posFlag' % argvs[0])
    quit()
#group=argvs[1]
target=argvs[1]
...
trmiFiles = ['../sql/JP.trmi.ret', '../sql/US.trmi.ret', '../sql/FR.trmi.ret', '../sql/GB.trmi.ret', '../sql/DE.trmi.ret','../sql/CN.trmi.ret', '../sql/CA.trmi.ret','../sql/CL.trmi.ret', '../sql/BR.trmi.ret', '../sql/AU.trmi.ret', '../sql/EG.trmi.ret', '../sql/IN.trmi.ret', '../sql/IR.trmi.ret', '../sql/IQ.trmi.ret', '../sql/IL.trmi.ret', '../sql/IT.trmi.ret', '../sql/LY.trmi.ret', '../sql/MX.trmi.ret', '../sql/KP.trmi.ret', '../sql/RU.trmi.ret', '../sql/ZA.trmi.ret', '../sql/CH.trmi.ret', '../sql/AR.trmi.ret', '../sql/UA.trmi.ret', '../sql/JPY.trmi.ret', '../sql/EUR.trmi.ret', '../sql/GBP.trmi.ret', '../sql/CHF.trmi.ret', '../sql/AUD.trmi.ret']
trmiGroup=['JP', 'US', 'FR', 'GB', 'DE', 'CN', 'CA','CL', 'BR', 'AU', 'EG', 'IN', 'IR', 'IQ', 'IL', 'IT', 'LY', 'MX', 'KP', 'RU', 'ZA', 'CH', 'AR', 'UA', 'JPY', 'EUR', 'GBP', 'CHF', 'AUD']
...
for i in range(prcLen - startFrom - dataSpan + 1,prcLen - dataSpan + 1,dataSpan):
    ...
    ret = asys.getSupervisingData(prc,trmi1,trmi2,trmi3,numDepth,posFlag)
    #ret = asys.getSupervisingMLPData(prc,trmi1,trmi2,trmi3,activationFunc,posFlag)
    ...
    plSubArray.append(lastTradePrice)
...
dfout=pd.DataFrame(plArray,columns=['timeStamp','pl','accPL','lastPrice'])
outname='./' + target + '-' + str(numDepth) + '-' + str(dataSpan) + '_' + str(posFlag) + '.csv'
#outname='./' + target + '-' + 'MLP' + '-' + '-' + str(activationFunc) + '-' + str(dataSpan) + '_' + str(posFlag) + '.csv'
dfout.to_csv(outname, index=False)

dfstd = dfout.std(ddof=False)
dfmean = dfout.mean()
