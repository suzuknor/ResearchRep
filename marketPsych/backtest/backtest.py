...
for i in range(0,startFrom - dataSpan + 1):
    ...
    pl = (nextTradePrc - lastTradePrc) * ret
    accPL = accPL + pl

    print(str(nextWindowTime) + ',: Prediction = ,' + str(ret) + ',===== Actual PL = ,' + str(pl) + ',===== Acumulative PL = ,' + str(accPL) + ',=====,' + str(lastTradePrc) + ',:,' + str(nextTradePrc), flush=True) 
